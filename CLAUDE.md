...
### Why the account carries a signing key per identity

One registration per signing identity rather than one per person: the
1Password-held key for the hosts that sign through it, one generated key per
host in `git_unattended_signing_hosts`, and the company-provisioned key for
repositories under the work directory prefix. Keys generated by this role are
per host and never copied, so one machine's key being compromised does not
implicate another's — which also means two hosts' keys share a filename and are
not the same key.

**Rejected, so it is not re-litigated: folding a host's own generated key into
the company-provisioned one.** It would mean one key on that host and one fewer
registration, with no prompt either way. Three reasons not to. Personal commits
would be signed by a key the employer provisioned, which is the wrong direction
for attributing your own work, more so with a move to company-managed accounts
expected. Every commit on the host would depend on an agent holding that key,
where today losing it breaks only work repositories. And it cuts against the
