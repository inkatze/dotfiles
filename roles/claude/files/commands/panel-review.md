...
- **copilot**: the Copilot CLI's non-interactive mode, with every tool withheld, from a fresh `mktemp -d` scratch directory in a subshell, same containment as gemini above. The lens prompt goes in `-p`; the tooling output and diff go on **stdin**:

  ```bash
  ( cd "$scratch" && "$copilot_bin" -s --available-tools '' --disable-builtin-mcps --disallow-temp-dir -p "$(cat "$prompt_file")" < "$payload_file" )
  backend_status=$?
  ```

  `--available-tools ''` leaves the model no tools at all, so it reviews only what it is given: measured on Copilot CLI 1.0.88, asked to read a file outside its input it answered that it had no access, and it needs no `--allow-all-tools` (which non-interactive mode otherwise asks for) because there is nothing to approve. `-s` prints only the response. It may open with a one-line preamble before the table; parse from the first table row. The diff goes to GitHub Copilot and uses the account's Copilot quota, which is why this backend is opt-in only.
