...
4. **Initialize iteration counter** = 0, and acquire a same-PR lock. The loop's per-iteration filter is `isResolved: false` (step (a)), so we don't need to snapshot HEAD or the baseline thread-ID set. Temp-file names elsewhere in this loop are namespaced by PR number, which prevents two nested runs on *different* PRs from clobbering each other but not two runs targeting the *same* PR (e.g., two worktrees or two sessions pointed at the same branch). A PID-based lock does not work here: each `Bash` tool call is a fresh process, so the process that wrote a lock has already exited by the time a later call would check it for liveness. Use a timestamp-with-refresh lock instead:
   ```bash
   lockfile=/tmp/copilot-review-nested-lock.NUMBER
   now=$(date +%s)
   if [ -f "$lockfile" ] && [ $(( now - $(cat "$lockfile" 2>/dev/null || echo 0) )) -lt 1800 ]; then
     echo "another nested copilot-review run appears active on PR NUMBER (lock is $(( now - $(cat "$lockfile") )) s old); stop rather than race it"
     exit 1
   fi
   echo "$now" > "$lockfile"
   ```
...
5. **Bootstrap a first review when the PR has none (zero threads, zero reviews).** Before entering the iteration loop, check the start state. When the PR's `reviewThreads` returns **zero** unresolved Copilot threads (step (a)'s query) **and** `reviews(last: 20)` (from step 3) shows **no** Copilot-authored review at all, the PR is brand-new to Copilot: neither Path A (code changes) nor Path B (already-handled / false-positive threads) has anything to act on, and step (a) has no threads to fetch. Bootstrap one review before the loop starts (bot-login detection for this same no-review state is handled by step 3's "Fallback when no Copilot review is found"; this step closes the loop-entry gap):
   ...
   - **Request the review.** Run step (f) (mode-aware: both modes try the native gh route first (gh ≥ 2.88.0), then the REST re-request with the mode's login form, and verify via the issue timeline's `review_requested` events; `app` mode falls back to the `request_copilot_review` MCP tool on a genuine 422). The **Re-review unavailable** stop condition applies if every transport fails — the native gh route failed or is unavailable, `app` mode's REST attempt genuinely 422s, and no `request_copilot_review` MCP tool is available — same as in the loop.
...
Apply the same rigor as interactive step 6 above, which nested mode replaces (canonical doctrine in CLAUDE.md `Validation Rigor (Solutions)`):
...
1. **Capture the baseline Copilot-review id and the poll-window start epoch, both before pushing, then commit and push.**
...
Branch on `repo_mode` from pre-flight step 3. Both modes use the same transport order (the native gh route first where gh ≥ 2.88.0, then REST), because the mode decides the login form, the read-back expectations, and the fallback path, never which transport is permitted. Three traps make working requests look broken; design around all of them:

- **gh ≥ 2.88.0 requests Copilot natively.** `gh pr edit NUMBER --add-reviewer @copilot` special-cases the `@copilot` handle and works in both modes with no MCP tool (verified twice 2026-08-31 on `inkatze/planwright#372`). Check `gh --version` once per run; on older gh, skip straight to REST.
- **Every requested-reviewer read-back hides Bot reviewers.** After a successful request, `GET /repos/{owner}/{repo}/pulls/{n}/requested_reviewers` returns `{"users":[],"teams":[]}`, and the GraphQL `reviewRequests` nodes come back just as empty (verified 2026-08-31 on `inkatze/planwright#372`: two successful requests, real Copilot reviews arrived, both read-backs empty throughout — an earlier revision of this step trusted `reviewRequests` and misdiagnosed a working request as "Copilot unavailable"). Neither read-back can distinguish success from a no-op. Verify via the issue timeline's `review_requested` events (query below), with step (g)'s poll as the authoritative confirmation.
- **The `[bot]` suffix behaves differently per API.** REST's `reviewers[]` array requires the suffixed form (`copilot-pull-request-reviewer[bot]`); spelling a raw bot login out to anything GraphQL-backed (including `gh pr edit --add-reviewer` with the login itself) fails with "Could not resolve user", bare or suffixed. Only the `@copilot` handle route above escapes this, because gh resolves the handle internally.

**`app` mode (`__typename == "Bot"` in pre-flight):** auto-review on push is **not** guaranteed. Verified failure mode (2026-05-02 live run on `SymmetrySoftware/stl-poc#13`): push completed, no auto-review fired, and step (g) would have timed out silently after 10 minutes. Do not skip the request.

Try the native gh route first (gh ≥ 2.88.0): `gh pr edit NUMBER --add-reviewer @copilot`. On exit 0, verify via the timeline query below and proceed to step (g). On failure, or gh < 2.88.0, try REST with the `[bot]`-suffixed login (substitute the verified bot login from pre-flight step 3 if it differs; verified working on an App-installed repo, `nihldev/tecpan#273`):
...
|---|---|---|
| 2xx | n/a | Verify via the timeline query below, then proceed to step (g). |
| 422 | `already requested` (or similar "duplicate reviewer") | **Treat as success.** A pending request means a review is coming: often GitHub's own auto-request on PR open (the bootstrap case), a prior iteration's still-live request, or another session's. Do **not** DELETE + re-POST here: no read-back can confirm what a DELETE actually did (the read-back trap above), and cancelling a pending request can kill a review Copilot is already producing. Verify via the timeline query below and proceed to step (g). |
| 422 | `not a collaborator` (or similar reviewer-rejection) | REST genuinely cannot request this reviewer here. Fall back to the GitHub MCP tool: `mcp__<github-server>__request_copilot_review` with `params: { owner, repo, pullNumber }` (the `<github-server>` substitute depends on the active MCP server, e.g. `claude_ai_Github-Symmetry`, `claude_ai_Github-Gusto`; the tool wraps an internal Copilot-review-request endpoint). If no such tool is available on the active server and the native gh route also failed (or gh < 2.88.0), stop with **Re-review unavailable** and report; do not assume push alone will trigger a review. |
| Other (4xx/5xx) | n/a | Log warning. Proceed to step (g); the poll is the authoritative signal. |

After the request returns success (the native gh route, REST 2xx, or the MCP fallback), **verify** the request was actually recorded, via the issue timeline and never a requested-reviewer read-back (both forms hide Bot reviewers, the read-back trap above):
...
If the latest `review_requested` event was created by this iteration's request and names the Copilot reviewer (its `requested_reviewer.login` is the User-facing `Copilot` or the verified bot login from pre-flight step 3), proceed to step (g). If no such event appears, log a warning ("reviewer request returned success but no review_requested timeline event records it") and proceed to step (g) anyway: the poll is the authoritative confirmation. **Step (g) is mandatory in app mode.** Skipping it on the assumption that the request "must have worked" is the regression earlier wording was written to prevent.

**`collaborator` mode:** try the native gh route first, same as app mode (gh ≥ 2.88.0: `gh pr edit NUMBER --add-reviewer @copilot`; on exit 0, verify via the timeline query above and proceed to step (g)). On failure, or gh < 2.88.0, try to trigger a new Copilot review by re-adding it to the requested reviewers list. Substitute the verified bot login from pre-flight step 3 if it differs from the default.
...
- Re-review request status: both paths. In `app` mode, report which transport step (f) used (the native gh route, REST, or the MCP fallback after a genuine 422), its outcome, and the timeline verification result; in `collaborator` mode, report the actual HTTP outcome from step (f). On Path B, also note whether step (g) returned NEW_REVIEW (Copilot re-reviewed unchanged HEAD) or TIMEOUT (the expected case that falls through to (f.5)).
...
1. **Address the in-scope threads as normal.** Run steps (b) validate, (c) implement, (d) local check, (e.1) capture baseline + poll-window epoch + commit + push, (e.3) reply, (e.4) submit any pending review, (e.5) resolve. Treat them exactly as you would an iteration with no out-of-scope threads.
...
- **Never** trust an external-effect step's happy-path response without re-querying state. After step (f)'s reviewer request (the native gh route, REST, or the MCP fallback), verify a `review_requested` timeline event recorded it (both the REST read-back and GraphQL `reviewRequests` omit Bot reviewers entirely, so neither can serve as the verification); after step (e.3)'s reply mutation, verify zero pending reviews owned by the viewer remain. Both bugs that motivated this section's wording (2026-05-02 live run on `SymmetrySoftware/stl-poc#13`) returned success and looked fine.

### After the loop

What happens next depends on why the loop ended:

- **Convergence** (unresolved-thread queue reached zero, via step (g) or the bootstrap's immediate-success exit). Present any remaining adjacent findings per "Handoff presentation" below.

  **If applying an adjacent finding changed code**, that change needs its own commit + push + re-request + poll cycle before the PR can be trusted as actually converged: capture the baseline id + poll-window epoch (same as step (e) Path A step 1), `git add` only the changed files, commit (`chore(copilot): iter N, apply adjacent finding <short summary>`), push, run step (f) to re-request review, then step (g) to poll, the normal Path A flow, incrementing the iteration counter as any review cycle would. If step (g) surfaces new unresolved threads, loop back to step (a) with those threads instead of continuing below.

  Once no adjacent-finding fix is outstanding (nothing was applied, or the cycle above confirmed zero unresolved threads), check `gh pr view --json isDraft,state -q '{isDraft: .isDraft, state: .state}'`. If this query fails (PR deleted, permissions lost, etc.), surface the error and hand off; do not assume draft or ready. If `state` is not `OPEN` (closed or merged in the meantime), stop here; there is nothing left to mark ready. If `isDraft` is `false`, stop here. Otherwise, re-fetch reviewThreads (same query as step (a)) to reconfirm the unresolved count is still zero: time has passed since convergence was declared, and a new thread could have landed in the interim. **If this recheck is nonzero**, do not proceed to the ready-ask: increment the iteration counter and loop back to step (a) with those threads, exactly as any other iteration would.

  **Only once the recheck confirms zero**, in an attended session, ask once, with the same weight as a Stop Conditions row: "Mark PR #<n> ready for review? Stop and wait for me." On yes, run `gh pr ready <number>` and, per the auto-execution invariant against trusting an external-effect step's happy-path response, re-query `gh pr view --json isDraft -q '.isDraft'` to confirm it actually flipped; if it is still `true` or the re-query fails, surface the failure and hand off rather than reporting success. On no, end the run with the PR left as a draft. **Dispatched or unattended** (no human at the prompt, e.g. invoked headlessly via planwright's `review_sequence`): do not ask and do not self-answer; leave the PR as a draft and hand off, naming the convergence outcome so the human can flip it ready themselves. This confirmation-gated ready-flip is the only PR-lifecycle action this loop takes, and only on this exit path.
