...
3. **Confirm the Copilot bot login and detect repo mode.** Default login is `copilot-pull-request-reviewer`. Verify by inspecting an existing review on the PR (`reviews(last: 20)` so the window is wide enough to surface a Copilot review even when several non-Copilot reviews stacked after the last Copilot one; `first: N` would return the oldest and may not include Copilot on a long-lived PR):
   ...
   ' -f owner='OWNER' -f repo='REPO' -F number=NUMBER
   ```
   Use the actual `Bot` login if it differs. (`commit { oid }` is the commit each review was submitted against; step (a)'s stale-baseline guard reads the most recent Copilot-authored node's value.)
...
- **SHAs differ, or the review's `commit` is null (the reviewed commit is gone from the branch)**: Copilot has not seen the current code, so a fresh review is required regardless of the zero count. Capture the baseline review id and poll-window epoch (the same two blocks as step (e) Path A step 1), run step (f) to request the review, then step (g) to poll. On `NEW_REVIEW`, loop back to step (a) with whatever threads it produced (step (g)'s normal counter handling applies); on `TIMEOUT`, trigger **No response** — a review was explicitly requested against unreviewed code and never arrived.
...
| 422 | `already requested` (or similar "duplicate reviewer") | DELETE the reviewer, re-POST (the same DELETE+POST pattern as collaborator mode below, with the `[bot]`-suffixed login). Then verify and proceed to step (g). |
...
After the request returns success (REST 2xx or the MCP fallback), **verify** Copilot is actually on the requested-reviewer list — via GraphQL, never REST read-back (the first trap above):
...
**Suppressed comments are invisible to the thread machinery — read them out of the body.** A Copilot review can report "no new comments" while its body carries a collapsed `Suppressed comments (N)` block containing a real finding (observed 2026-08-14 on `nihldev/tecpan#273`). Suppressed comments never become review threads, so every thread-based step — fetch, validate, reply, resolve, the convergence check — steps straight over them, and a run can legitimately report convergence while a substantive comment sits unread in the review body. That is the same false-green class the errored-review filter above exists to catch, arriving through a different door. On every `NEW_REVIEW`, read the captured review body and parse any suppressed-comments block; route each entry through step (b)'s three-pass validation and treat survivors as **adjacent findings** (they have no thread to reply to or resolve, so they flow into the handoff presentation like any other adjacent finding). The validation is not optional: in the observed case Copilot's individual facts were correct but its conclusion inverted under validation (it cited the repo's single outlier file as the project norm), so surfacing the block without validating would have propagated a wrong conclusion.
...
| **Re-review unavailable** | Step (f) `app` mode's REST attempt genuinely 422'd ("not a collaborator") AND no `request_copilot_review` MCP tool is available on the active server, so we cannot trigger a Copilot review and step (g)'s poll would never see one. A missing MCP tool alone does not fire this: REST is tried first. |
