...
2. Resolve base/head: base defaults to the current branch's usual base (the repo's default branch), or a `--base <ref>` argument if given; head is the working tree as-is (uncommitted included). `{head}` substitutes to `HEAD`, the committed tip, so a template that uses it reviews committed changes only; for a CLI that reviews the working tree when given no head, omit `{head}` from its template so uncommitted changes stay in scope. No PR is consulted for this, matching this mode's own "no `gh api` calls at all."
3. Create a fresh output directory for this run (`out="$(mktemp -d)" || exit 1`), then invoke `cli.local_invocation` with `{base}`, `{head}`, `{effort}`, and `{output}` substituted (`{output}` is that directory). The command owns the output location rather than the CLI's own default: a fixed path can serve a previous run's stale results, and a vendor's default (often a timestamped cache directory) would have to be rediscovered after every run. Adapt flag names to the real CLI's current `--help` output before first use (the config's invocation string is illustrative, not verified against any specific CLI). `{effort}` comes from the `--effort <value>` argument (mirrors `--base`); if a reviewer's `local_invocation` names `{effort}` and no `--effort` was given, do not run it with a literal `{effort}` and do not guess a value: stop and say this reviewer's template needs `--effort`. A template that should always run at one level can hardcode it instead of using `{effort}`. Bound the invocation with `cli.timeout_seconds`. On a non-zero exit or a timeout, print stderr (or "timed out after `cli.timeout_seconds`s") and stop; do not proceed to step 4's parse against partial or absent output.
4. Parse findings per `cli.findings_output`: `stdout-json`, or `file:<path>`, where `<path>` may reference `{output}` (e.g. `file:{output}/findings.json`) and should whenever the CLI accepts an output flag. Remove the run's output directory once parsed. Present as `File:Line | Finding | Severity`; no bucket categorization here, since nothing is applied automatically in this mode. **A zero exit does not guarantee parseable output**: if `stdout-json` doesn't parse, or a `file:<path>` the CLI was supposed to write is missing or empty, stop and say so rather than presenting an empty or partial findings table as "no findings."
