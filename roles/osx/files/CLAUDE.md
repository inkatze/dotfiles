...
- When a session ID was captured, closing the window doesn't lose the work: `claude --resume <session-id>` in a fresh window picks the conversation back up. Check `tmux-offload --list` for a launched session's ID (the `session_id` field is empty if discovery failed or was ambiguous).
