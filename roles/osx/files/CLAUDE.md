...
2. **Polish** (`/polish`): Autonomous loop of `/self-review`'s discovery + validation, applying only Auto-applicable items (tool-grounded, mechanical, no behavior change, validation passes converged). Hands off the moment any Needs human attention item appears or any safety condition fires. Local-only: no push, no PR. Use as a finishing pass before `/self-review` opens the PR.
