...
When the toolchain is clean and the diff is uniformly inside a categorization disqualifier (e.g., entirely security-sensitive auth code), polish will hand off on iteration 1 with all findings in Needs human attention. That is the expected outcome, not a failure.
