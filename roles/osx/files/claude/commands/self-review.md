...
6. Follow the standard review workflow (let me choose: all at once, one by one, batched decisions, or clustered decisions, with progress tracking). For clustered mode, the `/self-review` cluster-wide option set is **Address all / Defer all to follow-up / Dismiss all / Pick individually** (auto-added "Other" handles custom decisions; "Pick individually" drops into batched mode for that cluster only). When implementing fixes, apply the **two-or-three-angle solution validation** (canonical spec in CLAUDE.md `Validation Rigor (Solutions)`):
