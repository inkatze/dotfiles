...
| File | Read by | Holds |
|---|---|---|
| `host` | `scripts/playbook.sh`, the `/panel-review` and `/code-review` commands | This machine's inventory alias: `work`, `personal`, `alt` or `server`. An empty or whitespace-only file counts as absent, since an empty alias would run every inventory host. Any other value that is not one of those aliases (a group name like `all`, a pattern, anything with a character outside letters, digits, `_` and `-`), from the file or from `DOTFILES_HOST`, makes `scripts/playbook.sh` refuse to run. |
...
| `op-account` | `scripts/playbook.sh` | The 1Password account (sign-in address or user id) every `op` call should use, e.g. `my.1password.com`. Only needed where `op account list` shows more than one account — the normal state of a work Mac that carries a company tenant alongside a personal one. With two accounts and no selection, `op` fails every read with "multiple accounts found", which takes down the GitHub MCP sync and the Pushover credential read. An absent, empty or whitespace-only file exports nothing, so single-account hosts are unaffected; an already-exported `OP_ACCOUNT` wins. Any other file value that isn't an account identifier (one starting with a letter or digit and made only of letters, digits, `.`, `_`, `@` and `-`) makes `scripts/playbook.sh` refuse to run; an exported `OP_ACCOUNT` is not checked. |
...
resolves, and the review commands read it to pick a non-Anthropic backend.
Set it on every host except `work`:
