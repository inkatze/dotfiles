...
| `op-account` | `scripts/playbook.sh` | The 1Password account (sign-in address or user id) every `op` call should use, e.g. `my.1password.com`. Only needed where `op account list` shows more than one account — the normal state of a work Mac that carries a company tenant alongside a personal one. With two accounts and no selection, `op` fails every read with "multiple accounts found", which takes down the GitHub MCP sync and the Pushover credential read. An absent, empty or whitespace-only file exports nothing, so single-account hosts are unaffected; an already-exported `OP_ACCOUNT` wins. Any other value that isn't an account identifier (anything with a character outside letters, digits, `.`, `_`, `@` and `-`) makes `scripts/playbook.sh` refuse to run. |
